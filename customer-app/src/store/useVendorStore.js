 ...
 *
 * Separate from `useAuthStore` on purpose: the auth store is session-shaped and
 * lives for as long as the app does, while this one is the vendor flow's
 * working set and can be thrown away when the role flips back to CUSTOMER.
 *
 ...
 * refetch — the alternative (one server query per tab) makes "Accept" look
 * like it deleted the order.
 *
...
export const useVendorStore = create((set, get) => ({
  ...
  /** Clears the working set when the app drops back to the customer flow. */
  reset: () =>
    set({
      orders: [],
      ordersError: null,
  ...
  /** Shop profile lands in the auth store — it is session data, not view data. */
